...
# === Initial Population ===
population = [generate_individual(q_end=q_end_positions[0]) for _ in range(pop_size)]
cost_history = []
...
for gen in range(generations):
    ...
    while len(new_population) < pop_size:
        ...
        child = crossover(p1, p2, q_end=q_end_positions[0])
        if np.random.rand() < mutation_rate:
            child = mutate(child, mutation_rate, q_end=q_end_positions[0])
